...
{
    public static class GetProductsSlim
    {
        ...
        public class Handler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken token)
            {
                ...
                var items = await _dbContext.Inventories
                    .Where(x => x.CompanyId == request.CompanyId && x.InventoryType == InventoryType.Product && !x.IsDeleted
                                    && x.Status == InventoryStatus.Active
                                    && (dontFilter || x.Name.Contains(request.Search)))
                    .Select(x => new Model { Id = x.Id, Name = x.Name, Rate = x.UnitSalesPrice, Quantity = x.QuantityInStock }).ToListAsync();
