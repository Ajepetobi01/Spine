...
{
    public static class ImportBankTransactionFromMono
    {
        ...
        public class Handler : IRequestHandler<Command, Response>
        {
            ...
            public async Task<Response> Handle(Command request, CancellationToken token)
            {
                ...
                var descriptions = bankImports.Select(x => x.Description).ToList();
                var duplicate = 0;
                
                var count = request.Transactions.Count(x => !descriptions.Contains(x.Description));
                ...
                foreach (var item in request.Transactions)
                {
                    ...
                    bankTransaction.UserReferenceNo = refNo;
                    ...
                    transaction.UserReferenceNo = refNo;
                ...
                {
                    var retMessage = duplicate == 0 ? "Import successful" : $"Import successful with {duplicate} record(s) skipped";
                    return new Response(HttpStatusCode.Created, retMessage);
                }

                return new Response(duplicate > 0 ? $"No record uploaded. {duplicate} record(s) exist" : "No record uploaded");
            }
        }
