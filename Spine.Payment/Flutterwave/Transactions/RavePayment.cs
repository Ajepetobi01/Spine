using Spine.Payment.Flutterwave.Models;
...
namespace Spine.Payment.Flutterwave.Transactions
{
    public class RavePayment
    {
        private readonly string _secretKey;
        private readonly string _callBackUrl;
        public RavePayment(string secretKey, string callBackUrl)
        {
        ...
        public async Task<TransactionInitResponseDTO> InitializeTransaction(string tx_ref, string amount, string customerEmail, string customerPhone,
        ...
        public async Task<ResolveAccountNumberViewModel> ResolveAccountNumber(string account_number, string account_bank)
        {
            var _client = HttpFactory.InitHttpClient(RaveConstant.BASE_URL)
                     .AddAuthorizationHeader(RaveConstant.AUTHORIZATION_TYPE, _secretKey)
                     .AddMediaType(RaveConstant.REQUEST_MEDIA_TYPE)
            ...
            //Send the request
            var response = await _client.PostAsync($"accounts/resolve", content);

            var json = await response.Content.ReadAsStringAsync();

            return JsonSerializer.Deserialize<ResolveAccountNumberViewModel>(json);
        }
