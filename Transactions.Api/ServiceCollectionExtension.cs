    ...
    public static class ServiceCollectionExtension
    {
        ...
        public static void RegisterOtherServices(this IServiceCollection services, IConfiguration configuration)
        {
           ...
           x.UseSqlServer(configuration.GetConnectionString("SpineConnection"))); // reads connection string from config file
            ...
            services.AddScoped<ISerialNumberHelper, SerialNumberHelper>();
            ...
            services.AddScoped<IHttpClientFactoryService, PaystackClient>();
            services.AddHttpClient<PaystackClient>();

            services.AddScoped<IHttpClientFactoryService, MonoClient>();
            services.AddHttpClient<MonoClient>();

            services.AddMediatR(AppDomain.CurrentDomain.Load("Spine.Core.Transactions")); //AppDomain.CurrentDomain.GetAssemblies()
            services.AddAutoMapper(AppDomain.CurrentDomain.Load("Spine.Core.Transactions"));  //typeof(CustomerMappingProfile)
            ...
            var interfaceType = typeof(IExportModel);
            var exportEntityTypes = //Assembly.GetAssembly(typeof(ExportTransactions.Model))
                AppDomain.CurrentDomain.Load("Spine.Core.Transactions")
                    .GetTypes().Where(t => interfaceType.IsAssignableFrom(t));

            services.AddSingleton<IEntityAccessorCache, EntityAccessorCache>();

            services.AddTransient<IExcelGenerator>(x => new ExcelGenerator(new ExportHeadersHelper(
                                    x.GetService<IEntityAccessorCache>(), exportEntityTypes.ToArray())));

            services.AddTransient<ICsvGenerator>(x => new CsvGenerator(new ExportHeadersHelper(
                                    x.GetService<IEntityAccessorCache>(), exportEntityTypes.ToArray())));
        }
